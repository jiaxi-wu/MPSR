...
class RPNHead(nn.Module):
    ...
    def forward(self, x, closeup=False):
        if not closeup:
            features = []
            logits = []
            bbox_reg = []
            ...
            #x list[Tensor], Tensor: num_closeups x channel x 8 x 8
            logits = []
            for feature in x:
                t = F.relu(self.conv(feature))
                t = t[:, :, 3: -3, 3: -3]# only use center 2x2(last 3x3)
                cls_conv = self.cls_conv(t)
                cls_conv = cls_conv.view(cls_conv.size(0) * self.num_anchors, self.mid_channels, cls_conv.size(2), cls_conv.size(3))# batch x num_anchor x midchannel x 2 x 2
                cls_logits = self.cls_logits(cls_conv).view(-1, self.num_anchors, cls_conv.size(2), cls_conv.size(3))
                logits.append(cls_logits)
            return logits
...
class RPNModule(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None, closeups=None):
        """
        Arguments:
        ...
        if closeups is not None:
            closeup_objectness = self.head(closeups, closeup=True)
        else:
            closeup_objectness = None
    ...
    def _forward_train(self, anchors, objectness, rpn_box_regression, targets, closeup_objectness):
